...
def is_valid_ip(ip):
    ...
    return (
        len(parts) == 4 and
        all(p.isdigit() and 0 <= int(p) <= 255 for p in parts) and
        not ip.startswith(('192.', '127.', '255.', '0.', '234.', '251.'))  # filter local/fake IPs
    )
...
''')
            ...
            print('[×] Facebook Registration Failed:', reg)
    except Exception as e:
    ...
    try:
        ...
        for i in range(count):
            ...
            if email:
                ...
                print(f"[×] Failed to create email. Skipping account {i+1}")
    except ValueError:
